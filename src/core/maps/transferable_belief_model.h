...
bool TBM::operator==(const TBM& lr){
  for(size_t i = 0; i<MAX_BELIEF; ++i){
    if(std::abs(lr._beliefs[i] - this->_beliefs[i]) >= 1e-6)
      return false;
  }
  return true;
}
